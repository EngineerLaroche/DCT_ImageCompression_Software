...
public class Entropy {
		...
		if(runlength == 0 && value == 0) {
			...
			while(runlength >= 0) {
				if(runlength <= 15) {
					/* create bit value */
					int binary = pair[1];
					binary |= HUFFMAN_AC[AMPLITUDE][runlength][pair[0]] << pair[0];
					
					/* write the value to the buffer */
					write(binary, pair[0] + HUFFMAN_AC[SIZE][runlength][pair[0]]);
				} else {
					/* more to be written */
					write(ZRL, ZRL_SIZE);
				}				
	...
	private static int[] getSizeAmplitudePair(int value) {
		...
		if(value < 0) {
			pair[1] = ~Math.abs(value) & ((int)Math.pow(2, pair[0]) - 1);
		} else {
			pair[1] = value;
		}
	...
	private static int getValue(int size, int amplitude) {
		...
		if(amplitude < min) {
			...
			return -(~Math.abs(amplitude) & ((int)Math.pow(2, size) - 1));
		} else {
...
//		System.out.println("Run size\tCode length\tCode word");
//		System.out.println("0/0 (EOB)\t4\t1010");
//		for(int i = 0; i <= 0xf; i++) {
//			if(i == 0xf)
//				System.out.println("F/0\t11\t11111111001");
//			for(int j = 0; j < 0xa; j++) {
//				String runsize = Integer.toHexString(i).toUpperCase() + "/" + Integer.toHexString(j+1).toUpperCase();
//				String codelength = Integer.toString(HUFFMAN_AC[0][i][j]);
//				String codeword = Integer.toBinaryString(HUFFMAN_AC[1][i][j]);
//				int padding = HUFFMAN_AC[0][i][j] - codeword.length();
//				for(; padding > 0; padding--) {
//					codeword = "0" + codeword;
//				}
//				
//				System.out.println(runsize + "\t" + codelength + "\t" + codeword);
//			}
//		}
//	}
}
